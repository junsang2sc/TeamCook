...
TeamCooK은 스킬 택소노미 기반 팀 구성 웹 서비스다. 브랜드 표면은 다크 네이비(`{colors.canvas-dark}` `#485671`) 히어로 밴드와 밝은 캔버스(`{colors.canvas}`) 밴드가 교차하는 구조를 취하며, 로고에서 추출한 네 가지 색상 — 블루(`#4D9EED`) · 틸(`#4DC2A8`) · 앰버(`#FABF4B`) · 다크 네이비(`#485671`) — 이 브랜드 크롬의 전부다. 그라디언트(`{colors.accent-blue}` → `{colors.accent-teal}` → `{colors.accent-amber}`)는 히어로 헤드카피와 하이라이트 텍스트에만 사용한다.

타이포그래피는 NanumSquareNeo(디스플레이 산세리프, 굵기 1단계 상향 매핑)가 헤드라인과 본문을 담당하고, 같은 폰트를 모노 아이브로우 역할로도 사용한다. 헤드라인은 sentence-case; 섹션 레이블·버튼·테이블 헤더는 대문자 모노.

**Key Characteristics:**
- `{colors.primary}` (`#4D9EED` 블루) CTA 버튼이 모든 전환 타깃을 담당.
- 세 가지 색상 그라디언트 (`{colors.accent-blue}` → `{colors.accent-teal}` → `{colors.accent-amber}`)는 히어로 헤드카피 텍스트·강조 구간에만 사용. 아이콘 크기로 축소하거나 순서를 바꾸지 않는다.
- 대문자 모노 아이브로우(`{typography.mono-caps-eyebrow}`)는 섹션 레이블·버튼·테이블 헤더 전용.
- 카드 라디우스 `{rounded.sm}` 4 px 기준; hairline 테두리로 elevation 표현, 그림자는 사용하지 않는다.
- 다크(`{colors.canvas-dark}`)와 라이트(`{colors.canvas}`) 밴드 교차; 중간 회색 없음.

## Colors

### Brand & Accent
- **Primary Blue** (`{colors.primary}` — `#4D9EED`): 모든 CTA 버튼, 활성 상태, 링크 컬러. 로고의 "Team" 글자색.
- **Secondary Amber** (`{colors.secondary}` — `#FABF4B`): 하이라이트, 경고 배지, 보조 액션. 로고의 "C"·"K" 글자색.
- **Teal** (`{colors.accent-teal}` — `#4DC2A8`): 완료/성공 상태, 틸 배지, 스킬 커버리지 바 긍정 구간. 로고의 "o" 글자색.
- **Dark Navy** (`{colors.accent-navy}` — `#485671`): 히어로 배경, 다크 밴드, Navbar 배경. 로고 배경색.

### Surface
- **Canvas** (`{colors.canvas}` — `#FAFBFC`): 라이트 밴드 기본 배경 (Step1~5 콘텐츠 영역).
- **Surface** (`{colors.surface}` — `#EEF5FC`): 카드 내부 소프트 배경, 인풋 필드.
- **Hairline** (`{colors.hairline}` — `#C9DAEA`): 테이블 행 구분선, 카드 테두리, 배지 테두리.
- **Canvas Dark** (`{colors.canvas-dark}` — `#485671`): 히어로 밴드, Navbar, 다크 카드 배경. 로고 네이비와 동일.
- **Surface Dark Soft** (`{colors.surface-dark-soft}` — `#3a4760`): 다크 밴드 위 카드/배지 배경. canvas-dark보다 약간 어두운 레이어.
- **On Dark** (`{colors.on-dark}` — `#FAFBFC`): 다크 표면 위 모든 텍스트.

### Text
- **Ink** (`{colors.ink}` — `#1A1A1A`): 라이트 표면의 헤드라인·본문.
- **Body** (`{colors.body}` — `#6B7B8F`): 보조 텍스트 — 캡션, 설명, 레이블.
- **On Dark** (`{colors.on-dark}` — `#FAFBFC`): 다크 표면 위 모든 텍스트.

### Semantic
별도 에러/성공 팔레트를 정의하지 않는다. 위험 상태는 `#dc2626`(시스템 레드), 성공은 `{colors.accent-teal}`, 경고는 `{colors.secondary}`(앰버)를 프레임워크 기본값으로 사용한다.

### Brand Gradient
세 가지 색상 그라디언트: `{colors.accent-blue}` → `{colors.accent-teal}` → `{colors.accent-amber}`
(`135deg, #4D9EED 0%, #4DC2A8 50%, #FABF4B 100%`)

히어로 헤드카피의 강조 텍스트에만 사용. 단일 색상으로 축소하거나 순서를 바꾸거나 네 번째 색상을 추가하지 않는다.
...
### Do
- `{colors.primary}` (`#4D9EED` 블루)를 모든 주요 CTA 버튼에 사용한다. 화면당 하나의 주요 파란 버튼.
- 섹션 아이브로우와 버튼 레이블은 반드시 대문자 모노(`{typography.mono-caps-eyebrow}` / `{typography.mono-caps-button}`)로 표현한다.
- 브랜드 그라디언트(`{colors.accent-blue}` → `{colors.accent-teal}` → `{colors.accent-amber}`)는 히어로 헤드카피 강조 텍스트에만 사용한다. 아이콘이나 소형 UI에 사용하지 않는다.
- 표면은 `{colors.canvas-dark}` → `{colors.canvas}` 교차 리듬을 유지한다. 다크-라이트 대비가 elevation 역할을 담당한다.
- 카드·버튼 라디우스는 `{rounded.sm}` 4 px를 기준으로 사용한다.
- 카드 elevation은 hairline 테두리(`{colors.hairline}`)로만 표현한다. 그림자는 사용하지 않는다.
- 틸(`{colors.accent-teal}`)은 성공·완료 상태 전용으로 사용한다 (스킬 커버리지 고점, done 뱃지 등).
- 앰버(`{colors.secondary}`)는 경고·보조 강조 전용으로 사용한다.

### Don't
- 다섯 번째 액센트 컬러를 추가하지 않는다. 블루·틸·앰버·네이비 네 가지가 전체 팔레트다.
- 본문 단락에 모노 폰트를 사용하지 않는다. 모노는 레이블 전용이다.
- 라이트 표면 카드에 그림자를 추가하지 않는다. hairline 테두리와 표면 대비로 elevation을 표현한다.
- 브랜드 그라디언트의 색상 순서를 바꾸거나 네 번째 색상을 추가하지 않는다. 그라디언트는 고정 객체다.
- 주요 버튼을 full-pill(`{rounded.full}`) 형태로 만들지 않는다. CTA 형태는 `{rounded.sm}` 4 px 직사각형이다.
- 헤드라인을 대문자 모노로 설정하지 않는다. 대문자는 모노 전용, 헤드라인은 sentence-case 디스플레이 산세리프다.
- `{colors.primary}` 블루와 `{colors.secondary}` 앰버를 같은 섹션에서 동등하게 경쟁시키지 않는다. 블루가 항상 주역이다.
